  ...
  std::map<std::string, int> &name_to_index)
{
  ...
  int transition_table_indI;
  int transition_table_indJ;
    ...
    while (index < token.size())
    {
      if (input_index.find (token[index]) == input_index.end ())
      {
         if (last_accept_index != -1)
         {
            ...
            // case ID insert in symbol table
            if (is_ID(last_accept_type))
            {
              add_to_symbol_table(token, start_index, last_accept_index);
            }
...
void
Parser::add_to_symbol_table (std::string token, int start_index, int last_accept_index)
{
...
bool
Parser::is_accept_state(std::string state)
{
  return state != "-";
}
bool
Parser::is_ID(std::string token_type)
{
...
Parser::evaluate_exp(vector<string> &exp)
{
  ...
  // incase non-special (punctuation, keywords, data type) tokens
  if (is_exp_or_def(exp[0]))
  {
    // hold expression/definition name
    ...
    for (int i = 2; i < exp.size(); i++)
    {
      ...
      {
        while (!operators.empty() && !has_higher_prior(curr_token, operators.top()))
           execute_operation(operators, operands);
        // handle \( \)
    ...
    // stack may have some operands to execute
  ...
  // incase (punctuation, keywords, data type)
  else
  {
    evaluate_special_token(exp);
  }
}

bool
Parser::is_exp(std::string token)
{
  return (token == ":");
}

bool
Parser::is_exp_or_def (std::string token)
{
  return token != "{" && token != "[";
}

void
Parser::execute_operation (stack<std::string> &operators, stack<Nfa> &operands)
{
...
void
Parser::evaluate_special_token (vector<string> &exp)
{
